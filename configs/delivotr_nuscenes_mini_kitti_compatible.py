...
# KITTI-compatible configuration for nuScenes mini dataset
# This config allows testing KITTI-trained model on nuScenes data
...
# IMPORTANT: Order MUST match KITTI model training order exactly!
# KITTI order: pedestrian (0), cyclist (1), car (2)
...
test_pipeline = [
    dict(
        ...
        coord_type='LIDAR',
        load_dim=5,  # nuScenes has 5 dimensions
        use_dim=4,   # Only use first 4 to match KITTI (drop timestamp/intensity)
    ),
    dict(
...
data = dict(
    ...
    test=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file=data_root + 'nuscenes_infos_val_kitti_range.pkl',  # Filtered to KITTI range
        pipeline=test_pipeline,
        modality=input_modality,
        classes=class_names,
        test_mode=True,
        box_type_3d='LiDAR',
        with_velocity=False  # KITTI model doesn't predict velocity
    )
)
