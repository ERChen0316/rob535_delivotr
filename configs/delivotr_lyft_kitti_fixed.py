...
# FIXED: KITTI-compatible configuration for Lyft dataset
# This config allows testing KITTI-trained model on Lyft data
# Using KITTI's exact class order: Pedestrian, Cyclist, Car
...
model = dict(
    ...
    bbox_head=dict(
        ...
        in_channels=128,
        tasks=[
            # MUST match KITTI training order: Pedestrian, Cyclist, Car
    ...
    test_cfg=dict(
        ...
        max_per_img=500,
        max_pool_nms=False,
        min_radius=[4, 0.85, 12],  # KITTI order: pedestrian, bicycle, car
        score_threshold=0.1,
        pc_range=point_cloud_range[:2],
        out_size_factor=1,
        voxel_size=voxel_size[:2],
        ...
        pre_max_size=4096,
        post_max_size=500,
        nms_thr=0.2  # Lower NMS threshold to get more detections
    )
)
...
# CRITICAL: Must match KITTI model training order: Pedestrian, Cyclist, Car
